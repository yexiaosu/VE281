...
public:
    ...
    HashTable()
    {
        ...
        // Suppose the initial size of buckets are 20.
        buckets.resize(23);
    }
    ...
    Value &operator[](const Key &key)
    {
        // TODO
        size_t i = 0;
        size_t pos = hash_func(key, i);
        while (buckets.at(pos).type != BucketType::Empty)
        {
            if (key == buckets.at(pos).key)
            {
                return buckets.at(pos).val;
            }
            i++;
            pos = hash_func(key, i);
        }
        buckets.at(pos).key = Key();
        buckets.at(pos).val = Value();
        buckets.at(pos).type = BucketType::Occupied;
        num_elements++;
        // check whether to rehash
    ...
    size_t erase(const Key &key)
    {
        ...
        while (buckets.at(pos).type != BucketType::Empty)
        {
            if (key == buckets.at(pos).key)
            {
                buckets.at(pos).type = BucketType::Deleted;
                buckets.at(pos).key = Key();
                buckets.at(pos).val = Value();
                num_deleted++;
                num_elements--;
...
private:
    ...
    size_t hash_func(const Key &key, const size_t i)
    {
        Hasher hasher;
        return (hasher(key) % buckets.size() + i * i) % buckets.size();
    }
};
