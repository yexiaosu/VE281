...
protected:                                                                  // DO NOT USE private HERE!
    ...
    static constexpr size_t
            DEFAULT_BUCKET_SIZE = HashPrime::g_a_sizes[0];  // default number of buckets is 5
    ...
    size_t findMinimumBucketSize(size_t bucketSize) const {
        ...
        size_t para = static_cast<size_t> (static_cast<double>(tableSize + 1) / maxLoadFactor);
        if (para >= bucketSize) {
            bucketSize = para;
        }
        auto *result = std::lower_bound(HashPrime::g_a_sizes, HashPrime::g_a_sizes + 62, bucketSize);
        if (result != HashPrime::g_a_sizes + 62) {
            return *result;
        } else {
...
public:
     ...
     * Time Complexity: Amortized O(k)
     * @param key
     * @return the iterator of the value
     */
    Iterator find(const Key &key) {
        // TODO: implement this function
        size_t bkt = hashKey(key);
        if (!(buckets.at(bkt).empty())) {
            // Find the key-value pair
            for (auto itr = buckets.at(bkt).before_begin(); itr != buckets.at(bkt).end(); ++itr) {
                auto new_itr = itr;
                ++new_itr;
                if (new_itr != buckets.at(bkt).end() && new_itr->first == key) {
                    return Iterator(this, buckets.begin() + bkt, itr);
                }
            }
        }
        // Not find
        Iterator itr = Iterator(this, buckets.begin() + bkt, (buckets.begin() + bkt)->before_begin());
        itr.endFlag = true;
        return itr;
    }
    ...
    bool insert(const Iterator &it, const Key &key, const Value &value) {
        ...
        if (it.endFlag == true) {
            // Not find
            if (((double) (tableSize + 1) / (double) buckets.size()) > maxLoadFactor) {
                rehash(buckets.size());
                return (this->insert(key, value));
            } else {
                size_t first_pos = firstBucketIt - buckets.begin();
                size_t new_pos = hashKey(key);
                // Update firstBucketIt
                if (firstBucketIt == buckets.end() || new_pos < first_pos) {
                    // Inserted pair is the first element
                    firstBucketIt = buckets.begin() + new_pos;
                }
                // insert
                HashNodeList tmp = *(it.bucketIt);
                tmp.insert_after(it.listItBefore, p);
                tableSize++;
                return true;
            }
        } else {
    ...
    bool erase(const Key &key) {
        ...
        if (it.endFlag == false) {
            // Update firstBucketIt
            size_t first_pos = firstBucketIt - buckets.begin();
            Iterator itr = Iterator(it);
            size_t new_pos = hashKey(itr->first);
            ++itr;
            // Erase
            HashNodeList tmp = *(it.bucketIt);
            tmp.erase_after(it.listItBefore);
            tableSize--;
            if (first_pos == new_pos && tmp.empty()) {
                firstBucketIt = itr.bucketIt;
            }
            return true;
        }
        return false;
    ...
    Value &operator[](const Key &key) {
        ...
        if (itr.endFlag == false) {
            return itr->second;
        } else {
            insert(itr, key, Value()); // has already rehashed and updated
            itr = find(key);
            return itr->second;
        }
    }
    ...
    void rehash(size_t bucketSize) {
        ...
        // TODO: implement this function
        HashTable<Key, Value, Hash, KeyEqual> newTable = HashTable(bucketSize);
        newTable.setMaxLoadFactor(this->maxLoadFactor);
        for (auto &it:(*this)) {
